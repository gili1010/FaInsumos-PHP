...
$alert = '';
session_start();
if (!empty($_SESSION['active'])) { //SI TIENE ALGO SESSION-ACTIVE 
  header('location: sistema/'); //ME LLEVA EL HEADER A LA PAGINA PRINCIPAL DE SISTEMA. 
} else { //Y SINO SE EJECUTA LO SIGUIENTE
  if (!empty($_POST)) { //SI NO ESTA VACIO EL POST
    if (empty($_POST['usuario']) || empty($_POST['clave'])) { //SI ESTA VACIO POST DE USUARIO Y CONTRA , APARECE EL MENSAJE
